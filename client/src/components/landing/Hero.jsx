...
import { FaStethoscope, FaHospital, FaUserMd, FaAward, FaShieldAlt, FaClock, FaGlobe, FaStar } from 'react-icons/fa';
          ...
          <div className="lg:col-span-7 text-center lg:text-left">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-gray-700">{t('hero.features.expertDoctors')}</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center mx-auto mb-3">
                  <FaClock className="text-green-600 text-xl" />
                </div>
                <p className="text-sm font-medium text-gray-700">{t('hero.features.available247')}</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center mx-auto mb-3">
                  <FaGlobe className="text-purple-600 text-xl" />
                </div>
                <p className="text-sm font-medium text-gray-700">{t('hero.features.globalReach')}</p>
              </div>
              <div className="text-center">
                <div className="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center mx-auto mb-3">
                  <FaAward className="text-yellow-600 text-xl" />
                </div>
                <p className="text-sm font-medium text-gray-700">{t('hero.features.awardWinning')}</p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-8 relative">
              ...
              <div className="mb-6">
                <div className="flex items-center mb-4">
                  ...
                  />
                  <div>
                    <h3 className="font-semibold text-gray-900">Ahmed Al-Rashid</h3>
                    <p className="text-gray-600 text-sm">Heart Valve Replacement • UAE</p>
                    <div className="flex items-center mt-1">
                      {[...Array(5)].map((_, i) => (
                        <FaStar key={i} className="text-yellow-400 text-sm" />
                      ))}
                      <span className="text-xs text-gray-500 ml-2">Verified Patient</span>
                    </div>
                  </div>
                </div>
                
                <blockquote className="text-gray-700 italic mb-6">
                  "ShafiMed connected me with Apollo Hospital in Chennai. The surgery was successful and I saved over $35,000 compared to local costs. Their team handled everything from visa to recovery accommodation."
                </blockquote>
              </div>
        ...
        <div className="mt-16 pt-16 border-t border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div>
              ...
              <div className="text-gray-600">{t('hero.stats.patientsTreated')}</div>
            </div>
            <div>
              <div className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">500+</div>
              <div className="text-gray-600">{t('hero.stats.expertDoctors')}</div>
            </div>
            <div>
              <div className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">50+</div>
              <div className="text-gray-600">{t('hero.stats.specializations')}</div>
            </div>
            <div>
              <div className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">24/7</div>
              <div className="text-gray-600">{t('hero.stats.supportAvailable')}</div>
            </div>
          </div>
        </div>
      </div>
