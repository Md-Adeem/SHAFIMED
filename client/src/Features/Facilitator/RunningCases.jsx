...
const RunningCases = () => {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border text-sm">
          <thead className="bg-gray-100 dark:bg-gray-700">
            <tr>
              {["Ref ID", "Patient", "Doctor", "Department", "Status", "Actions"].map((head, i) => (
                <th key={i} className="px-4 py-2 text-left text-gray-600 dark:text-gray-300">
                  {head}
                </th>
              ))}
            </tr>
  ...
  return (
    <FacilitatorLayout
      ...
      actions={
        !loading && (
          <div className="flex flex-wrap items-center gap-3 text-gray-900 dark:text-gray-100">
            <select
              value={selectedStatus}
              onChange={(e) => setSelectedStatus(e.target.value)}
              className="border rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600"
            >
          ...
          <table className="min-w-full border text-sm">
            ...
            <tbody>
              {filteredCases.map((c) => (
                ...
                >
                  <td className="px-4 py-2 text-gray-900 dark:text-gray-100">{c.referenceId || "N/A"}</td>
                  <td className="px-4 py-2 text-gray-900 dark:text-gray-100">{c.patientId?.name || "N/A"}</td>
                  <td className="px-4 py-2 text-gray-900 dark:text-gray-100">{c.assignedDoctorId?.name || "N/A"}</td>
                  <td className="px-4 py-2 text-gray-900 dark:text-gray-100">{c.department || "N/A"}</td>
