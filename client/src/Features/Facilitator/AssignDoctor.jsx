...
export default function AssignDoctor({ queryId, onAssigned }) {
  ...
  useEffect(() => {
    const loadDoctors = async () => {
       try {
    const token = localStorage.getItem('token');  // Example of getting token from localStorage
    const { data } = await api.get("/queries/doctors/list", {
      headers: {
        Authorization: `Bearer ${token}`,  // Include Bearer token
      }
    });
    console.log("Doctors fetched:", data); // Check the data returned
    setDoctors(data);
  } catch (err) {
  ...
  const handleAssign = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`/queries/${queryId}`, { assignedDoctorId: selected, status: "Assigned" });
